...
t0 = time.time()
test_sector = 'Basic Materials' #Include Utilities, Consumer Defensive, Industrials,
...
fundamentals = fundamentals.drop_duplicates(subset = ['ticker', 'calendardate'], keep='first')
...
fundamentals['Sector_YoY_Earnings_Average'] = \
    fundamentals['Sector YoY Earnings Average']
...
fundamentals['Balance_Sheet_Aggregrate_Accruals_den'] = \
    fundamentals.groupby('ticker')['NOA'].apply(lambda x: x.rolling(center=False, window=2).sum())
...
fundamentals['workingcapital_lag'] = \
    fundamentals['workingcapital'].shift(lag_number)
fundamentals['capex_to_sales_lag'] = \
    fundamentals['capex_to_sales'].shift(lag_number)
fundamentals['YoY_Rev_Growth_lag'] = \
    fundamentals['YoY_Rev_Growth'].shift(lag_number)
fundamentals['YoY_NetInc_Growth_lag'] = \
    fundamentals['YoY_NetInc_Growth'].shift(lag_number)
fundamentals['YoY_NCFO_Growth_lag'] = \
    fundamentals['YoY_NCFO_Growth'].shift(lag_number)
...
OOS_labels = np.array(features['ebitda'])
# Remove the labels from the feature and the date
# axis 1 refers to the columns
OOS_features_array = features.drop(['ebitda', 'calendardate'], axis = 1)
...
path_to_file = (
    ...
    ' Code, and Results/Random Forest Models/'
    )
...
Sector_R2_score = 'OOS R^2 Score EVEBITDA'+test_sector
...
Sector_MAE = 'OOS MAE EVEBITDA'+test_sector
...
features_2 = fundamentals_2[['calendardate', 'ev',
                             'ebitda',
                             'ticker',
                             'price',
                             'marketcap',
...
features_4['Forward EBITDA/EV'] = features_4['12 Mo Forecast EBITDA']/features_4['ev']
...
path_to_file = (
    r'/Users/downey/coding/Python/Scripts/EPAT Project RF Models,'
    ' Code, and Results/Random Forest Models/PFCF Forecasts/'
    )
output_name = path_to_file + test_sector + '_OOS_EVEBITDA_Predictions.csv'
