...
os.chdir(
    '/Users/downey/coding/Python/Scripts/'
    'EPAT Project RF Models, Code, and Results'
    )

##############################################################################

#Random Forests to Predict 12 months ahead EBITDA
...
#%%
t0 = time.time()
test_sector = 'Utilities' #you will need to cycle through each sector
...
fundamentals['Sector YoY Earnings Average'] = fundamentals['calendardate'].map \
    (Dictionary_Industry_values)
...
fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector'] = \
    fundamentals['Company_YoY_NetInc_Growth']/ fundamentals['Sector YoY Earnings Average']
fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector'] = \
    fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector']
...
fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector'] = \
    fundamentals['Company_YoY_Revenue_Growth']/ fundamentals['Sector YoY Revenue Average']
fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector'] = \
    fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector']
...
features = fundamentals[['calendardate', 'ebitda',
                         'fcf_lag',    
                         'Revenue_lag',
                         'capex_lag',
                         'de_lag',
...
labels = np.array(features['ebitda'])
# Remove the labels from the feature and the date
features_array = features.drop(['ebitda', 'calendardate'], axis=1)
...
rf_EVEBITDA = RandomForestRegressor(n_estimators=1000, random_state=42, \
                           criterion="mse", max_depth=None, \
                           max_features=round(number_features/3))

# Train the model on training data
rf_EVEBITDA.fit(train_features, train_labels)
...
Sector_RF_Model = 'RF_Model_EVEBITDA_'+test_sector

filename = path_to_file + Sector_RF_Model + '.sav'
pickle.dump(rf_EVEBITDA, open(filename, 'wb'))

# Use the forest's predict method on the test data
predictions = rf_EVEBITDA.predict(train_features)
...
# Use the forest's predict method on the test data
predictions_val = rf_EVEBITDA.predict(validation_features)

#predicting a few earnings
rf_EVEBITDA.predict(validation_features[-2:-1])
...
score_R2_train = rf_EVEBITDA.score(train_features, train_labels) #train set
score_R2_validation = rf_EVEBITDA.score(validation_features, validation_labels) #validation set

R2_df = pd.DataFrame({'R^2 Score Train': score_R2_train,
                      'R^2 Score Validation': score_R2_validation}, index=[test_sector])
print(R2_df)

Sector_R2_score = 'IS R^2 Score EVEBITDA'+test_sector
...
Sector_MAE = 'MAE EVEBITDA '+test_sector
...
with open('/Users/downey/coding/Python/Scripts/EPAT Project RF Models, '
          'Code, and Results/feature_list_EVEBITDA.txt', 'wb') as fp:
...
features_2 = fundamentals_2[['calendardate', 'ev', 'ebitda', 'ticker',
                             'price', 
                             'marketcap',
                             'fcf_lag',
                             'Revenue_lag',
...
predictions_val = rf_EVEBITDA.predict(validation_features)
predictions_train = rf_EVEBITDA.predict(train_features)

forecasted_values = np.append(predictions_train, predictions_val)

#forecasted_12mo_EBITDA
forecasted_12mo_EBITDA = pd.Series(forecasted_values, name='12 Mo Forecast EBITDA')

features_3 = features_3.reset_index()

features_4 = pd.concat((features_3, forecasted_12mo_EBITDA), axis=1, join='inner')

#create forward EV/EBITDA
features_4['Forward EBITDA/EV'] = features_4['12 Mo Forecast EBITDA']/ \
    features_4['ev']

#confirming that the estimates are aligned correctly
rf_EVEBITDA.predict(train_features[1:2, :])
features_4['12 Mo Forecast EBITDA'].head()


rf_EVEBITDA.predict(validation_features[-2:-1, :])
features_4['12 Mo Forecast EBITDA'].tail()

Data_for_Portfolio_EVEBITDA = features_4[['calendardate', 'ticker',
                                          '12 Mo Forecast EBITDA',
                                          'Forward EBITDA/EV']]
...
output_name = path_to_file + test_sector + '_EVEBITDA_Predictions.csv'
Data_for_Portfolio_EVEBITDA.to_csv(output_name)

t1 = time.time()
print('It took ' + str((t1-t0)/60) + ' minutes to run the code')

os.system("say 'Your EBITDA to EV Code is Done'")
#%%
