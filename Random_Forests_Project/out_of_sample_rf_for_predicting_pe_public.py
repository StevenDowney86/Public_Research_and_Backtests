...
fundamentals['Company_YoY_Revenue_Growth'] = fundamentals.groupby\
    ('ticker')['revenue'].pct_change(4)
...
OOS_labels = np.array(features['netinc'])
# Remove the labels from the feature and the date
# axis 1 refers to the columns
OOS_features_array= features.drop(['netinc', 'calendardate'], axis = 1)
...
Sector_R2_score = 'OOS R^2 Score PE'+test_sector
...
Sector_MAE = 'OOS MAE PE'+test_sector
...
features_2 = fundamentals_2[['calendardate', 'netinc',
                             'ticker',
                             'price',
                             'marketcap',
                             'fcf_lag',
...
forecasted_12mo_Earnings = pd.Series(forecasted_values, name='12 Mo Forecast Earnings')

features_3 = features_3.reset_index()

features_4 = pd.concat((features_3, forecasted_12mo_Earnings), axis=1, join='inner')

#create forward Earnings/P
features_4['Forward E/P'] = features_4['12 Mo Forecast Earnings']/features_4['marketcap']
...
output_name = path_to_file + test_sector + '_OOS_PE_Predictions.csv'
