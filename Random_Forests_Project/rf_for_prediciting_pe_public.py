...
#Random Forests to Predict 12 months ahead Earnings
...
#create a 4 quarter lag as 4 quarters ago fundamentals are all we have to
#forecast next full years earnings
...
fundamentals['assetturnover_lag'] = fundamentals['assetturnover'].\
    shift(lag_number)
...
features = fundamentals[['calendardate', 'netinc',
                         'fcf_lag',
                         'Revenue_lag',
                         'capex_lag',
                         'de_lag',
...
labels = np.array(features['netinc'])
# Remove the labels from the feature and the date
features_array = features.drop(['netinc', 'calendardate'], axis=1)
...
rf_PE = RandomForestRegressor(n_estimators=1000, random_state=42, \
                           criterion="mse", max_depth=None, \
                           max_features=round(number_features/3))

# Train the model on training data
rf_PE.fit(train_features, train_labels)
################Accuracy Scoring#########################################
...
Sector_RF_Model = 'RF_Model_PE_'+test_sector
filename = path_to_file + Sector_RF_Model + '.sav'
pickle.dump(rf_PE, open(filename, 'wb'))

# Use the forest's predict method on the test data
predictions = rf_PE.predict(train_features)
...
# Use the forest's predict method on the test data
predictions_val = rf_PE.predict(validation_features)

#predicting a few earnings
rf_PE.predict(validation_features[-2:-1])
...
score_R2_train = rf_PE.score(train_features, train_labels) #train set
score_R2_validation = rf_PE.score(validation_features, validation_labels) #validation set

R2_df = pd.DataFrame({'R^2 Score Train': score_R2_train,
                      'R^2 Score Validation': score_R2_validation}, index=[test_sector])

Sector_R2_score = 'IS R^2 Score PE'+test_sector
...
with open('/Users/downey/coding/Python/Scripts/EPAT Project RF Models, '
          'Code, and Results/feature_list_PE.txt', 'wb') as fp:
...
features_2 = fundamentals_2[['calendardate', 'netinc', 'ticker', 'price',
                             'marketcap',
                             'fcf_lag',
                             'Revenue_lag',
                             'capex_lag',
...
predictions_val = rf_PE.predict(validation_features)
predictions_train = rf_PE.predict(train_features)

forecasted_values = np.append(predictions_train, predictions_val)

#forecasted_12mo_earnings
forecasted_12mo_earnings = pd.Series(forecasted_values, \
                                     name='12 Mo Forecast Earnings')

features_3 = features_3.reset_index()

features_4 = pd.concat((features_3, forecasted_12mo_earnings), \
                       axis=1, join='inner')

#create forward P/E
features_4['Forward E/P'] = features_4['12 Mo Forecast Earnings']/ \
    features_4['marketcap']

#confirming that the estimates are aligned correctly
rf_PE.predict(train_features[1:2, :])
features_4['12 Mo Forecast Earnings'].head()

rf_PE.predict(validation_features[-2:-1, :])
features_4['12 Mo Forecast Earnings'].tail()
 
Data_for_Portfolio_PE = features_4[['calendardate', 'ticker',
                                    '12 Mo Forecast Earnings',
                                    'Forward E/P']]
...
output_name = path_to_file + test_sector + '_PE_Predictions.csv'
Data_for_Portfolio_PE.to_csv(output_name)

t1 = time.time()
print('It took ' + str((t1-t0)/60) + ' minutes to run the code')

os.system("say 'Your Earnings to Price Code is Done'")
#%%
