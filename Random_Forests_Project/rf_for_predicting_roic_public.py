...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 13:34:58 2020
...
quandl.ApiConfig.api_key = "keyhere"
...
#Random Forests to Predict 12 months ahead Return on Invested Capital
...
#fitler out the tickers based on specified sector, industry, or highest market cap
...
fundamentals['Sector YoY Earnings Average'] = fundamentals['calendardate'].map(Dictionary_Industry_values)

fundamentals['Sector_YoY_Earnings_Average'] = fundamentals['Sector YoY Earnings Average']

fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector'] = fundamentals['Company_YoY_NetInc_Growth']/ fundamentals['Sector YoY Earnings Average']
fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector'] = fundamentals['Ratio_Comp_YoY_Earn_Rate_vs_Sector']

####    Creating Sales Growth Base Case    ####
fundamentals['Company_YoY_Revenue_Growth'] = fundamentals.groupby('ticker')['revenue'].pct_change(4)
...
#map the values of the dictionary onto the calendardate keys in the dataset
fundamentals['Sector YoY Revenue Average'] = fundamentals['calendardate'].map(Dictionary_Industry_values)
fundamentals['Sector_YoY_Revenue_Average'] = fundamentals['Sector YoY Revenue Average']

fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector'] = fundamentals['Company_YoY_Revenue_Growth']/ fundamentals['Sector YoY Revenue Average']
fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector'] = fundamentals['Ratio_Comp_YoY_Reve_Rate_vs_Sector']
...
labels = np.array(features['NOPAT'])
# Remove the labels from the feature and the date
features_array = features.drop(['NOPAT', 'calendardate'], axis=1)
...
rf_ROIC = RandomForestRegressor(n_estimators=1000, random_state=42, \
                           criterion="mse", max_depth=None, \
                           max_features=round(number_features/3))

# Train the model on training data
rf_ROIC.fit(train_features, train_labels)
################Accuracy Scoring#########################################
...
pickle.dump(rf_ROIC, open(filename, 'wb'))

# Use the forest's predict method on the test data
predictions = rf_ROIC.predict(train_features)
...
# Use the forest's predict method on the test data
predictions_val = rf_ROIC.predict(validation_features)

#predicting a few earnings
rf_ROIC.predict(validation_features[-2:-1])
...
#compute R squared for datas
score_R2_train = rf_ROIC.score(train_features, train_labels) #train set
score_R2_validation = rf_ROIC.score(validation_features, validation_labels) #validation set

R2_df = pd.DataFrame({'R^2 Score Train': score_R2_train,
                      'R^2 Score Validation': score_R2_validation}, \
                     index=[test_sector])

Sector_R2_score = 'IS R^2 Score ROIC'+test_sector
...
Sector_MAE = 'MAE ROIC '+test_sector
...
with open('/Users/downey/coding/Python/Scripts/EPAT Project RF Models, '
          'Code, and Results/feature_list_ROIC.txt', 'wb') as fp:
    pickle.dump(feature_list, fp)

##########################Forming the Forecasts#############################3
...
predictions_val = rf_ROIC.predict(validation_features)
predictions_train = rf_ROIC.predict(train_features)

forecasted_values = np.append(predictions_train,predictions_val)

#forecasted_12mo_ROIC
forecasted_12mo_NOPAT = pd.Series(forecasted_values, name='12 Mo Forecast NOPAT')

features_3 = features_3.reset_index()

features_4 = pd.concat((features_3,forecasted_12mo_NOPAT), axis=1, join='inner')
...
rf_ROIC.predict(train_features[1:2,:])
...
output_name = path_to_file + test_sector + '_ROIC_Predictions.csv'
Data_for_Portfolio_ROIC.to_csv(output_name)

t1 = time.time()
print('It took ' + str((t1-t0)/60) + ' minutes to run the code')

os.system("say 'Your Return on Invested Capital Code is Done'")
#%%
